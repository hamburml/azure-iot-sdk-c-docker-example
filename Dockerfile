...
# clone azure (use dps_symm_key branch for IoT Central Support)
RUN git clone -b dps_symm_key --recursive https://github.com/Azure/azure-iot-sdk-c.git
...
RUN echo "SET(CMAKE_CXX_COMPILER ${TOOLCHAIN_EXES}/${TOOLCHAIN_NAME}-gcc)" >> toolchain.cmake
...
RUN cmake -DCMAKE_TOOLCHAIN_FILE=toolchain.cmake -Duse_prov_client:BOOL=ON -DCMAKE_INSTALL_PREFIX=${TOOLCHAIN_PREFIX} ..
RUN make
RUN make install
# or RUN cmake --build .
...
WORKDIR example
ADD CMakeLists.txt .
ADD prov_dev_client_ll_sample.c .
RUN mkdir cmake
WORKDIR cmake
